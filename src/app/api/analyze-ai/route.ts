...
export async function POST(request: Request) {
  ...
  try {
    const inference = await runInferenceGateway({
      ...
      workspaceId,
      projectId,
      responseFormat: { type: "json_schema", jsonSchema: ANALYSIS_JSON_SCHEMA },
      temperature: 0.2,
      messages: [
...
${extractedScopeText.slice(0, 16000)}` },
      ...
      metadata: { estimatedSensitivity: "internal", dataClasses: ["project_scope"], moduleId: "analyze-ai" },
    });

    const analysis = coerceAnalysisResult(inference.parsedJson ?? JSON.parse(inference.content));
